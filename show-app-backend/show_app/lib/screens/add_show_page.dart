...
class _AddShowPageState extends State<AddShowPage> {
  ...
  Future<void> _addShow() async {
    ...
    if (response.statusCode == 201) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("Show added successfully!")));
      Navigator.pop(context, true); // Return to refresh home page
    } else {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("Failed to add show")));
    }
  }
        ...
        child: Column(
          ...
          children: [
            ...
            _imageFile == null
                ...
                : Image.network(
                  '${ApiConfig.baseUrl}/uploads/${_imageFile!.path.split('/').last}', // Use the full image URL
                  height: 150,
                  fit: BoxFit.cover,
                ),
            Row(
